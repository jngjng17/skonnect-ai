...
# Normalize string columns in event_df
...
if not os.path.exists(LOG_FILE):
    pd.DataFrame(columns=[
        "timestamp", "user_message", "predicted_intent",
...
def log_conversation(user_message, predicted_intent, bot_reply, source="keras"):
    ...
    log_df = pd.DataFrame([[
        timestamp, user_message, predicted_intent, bot_reply, source
    ]], columns=[
        "timestamp", "user_message", "predicted_intent",
        "bot_response", "model_source"
    ])
    ...
    # Incremental retraining from logs
    try:
...
def recommend_event_all(category, limit=None):
    if not category:
        return []
    mask = event_df["Recommended Category"].astype(str).str.lower().str.strip() == category.strip().lower()
...
def chat():
    ...
    bot_reply, recommendations = "I'm not sure how to respond yet.", []

    if model_type == "faq":
        if predicted_intent in faq_df["intent"].values:
            responses = faq_df[faq_df["intent"] == predicted_intent]["bot_response"].tolist()
            if responses:
                bot_reply = generate_dynamic_reply(random.choice(responses), predicted_intent)
            else:
        ...
        if categorized_interests:
            ...
            if recommendations:
                ...
                bot_reply = (
                    f"Based on your interests in **{', '.join(categorized_interests)}**, "
